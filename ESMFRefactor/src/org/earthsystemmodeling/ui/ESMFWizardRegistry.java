...
public class ESMFWizardRegistry { //extends CWizardRegistry {

	public final static String ESMF_WIZARD_CATEGORY_ID = "org.earthsystemmodeling.wizards"; //$NON-NLS-1$
	...
	public static boolean isFortranProjectWizard(IConfigurationElement element) {
	    ...
	    return (category != null && category.equals(ESMF_WIZARD_CATEGORY_ID));
	}
		...
		if (extensionPoint != null) {
			...
			for (int i = 0; i < elements.length; i++) {
				...
				if (element.getName().equals(TAG_WIZARD)) {
				    String category = element.getAttribute(ATT_CATEGORY);
				    if (category != null && category.equals(ESMF_WIZARD_CATEGORY_ID)) {
			            //String id = element.getAttribute(TAG_ID);
			            elemList.add(element);
				    }
				}
